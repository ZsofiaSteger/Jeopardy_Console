...
def display_questions(active_questions):
    ...
    for active_question in active_questions.itertuples():
        console.print(
            Panel(
                f"[bold cyan]{active_question.value}[/bold cyan], "
                + f"[bold bright_green]Category: {active_question.category}[/bold bright_green]",
                title=f"Question {i}",
                title_align="left",
...
def check_answer(user_answer, correct_answer):
    if fuzz.ratio(user_answer, correct_answer) > 80:
        console.print(
            f"[bold green]Correct![/bold green] The answer was: {correct_answer}"
        )
        return True
    else:
        console.print(
            f"[bold red]Incorrect.[/bold red] The correct answer was: {correct_answer}"
        )
        return False
    ...
    for round in range(0, number_of_rounds):
        ...
        console.print(
            Panel(
                ...
                title_align="left",
                width=LARGE_PANEL_WIDTH,
            )
        )

        user_answer = input("Enter your answer: ")
        answer_status = check_answer(
            user_answer, active_questions.iloc[chosen_one].answer
        )
        score = get_score(
            answer_status,
            score,
            int(
                active_questions.iloc[chosen_one]
                .value.replace("$", "")
                .replace(",", "")
            ),
        )
        console.print(
            Panel(
                f"[bold yellow]Your score is: {score}[/bold yellow]",
                width=SMALL_PANEL_WIDTH,
            )
        )
